...
export async function createEngineeringReport(snapshot, en, fontBytes) {
  ...
  const units={'UAH':'грн','UAH/h':'грн/год','UAH/t':'грн/т','UAH/t-%':'грн/т-%','UAH/kWh':'грн/кВт·год','UAH/season':'грн/сезон','t':'т','t/h':'т/год','h':'год','MJ':'МДж','kWh':'кВт·год'};
  ...
  pdf.setTitle(`ARQON ${snapshot.modelName} — ${t('Технічний підсумок','Technical summary')}`);pdf.setAuthor('ARQON.UA');
  ...
  function start(title) {
    ...
    page.drawRectangle({x:0,y:738,width:595.28,height:104,color:rgb(.97,.94,.95)});text('A',42,786,25,rgb(.78,.06,.17));text('RQON.UA',58,786,25);text(snapshot.modelName,450,790,19,orange);
  ...
  row(t('Обслуговування та постійні витрати','Maintenance and fixed costs'),n(i.maintenancePerSeason,'UAH/season'));
  ...
  row(t('Продуктивність за висушеним зерном','Dried grain capacity'),n(r.capacity,'t/h'));
  ...
  row(t('Собівартість на тонну вхідного зерна','Drying cost per incoming tonne'),n(r.own?.perTonne,'UAH/t'));
  paragraph(t('Витрата палива розрахункова, не паспортна. ККД: газ/дизель 90%, щепа 65%; втрати 5%, потім рекуперація 20%. Нагрівається суха речовина до 50 °C. Для соняшнику — до 45 °C. Час роботи визначається за масою висушеного зерна. Потужність електродвигунів уже враховує коефіцієнт 0,8.','Fuel consumption is calculated, not a rated specification. Efficiency: gas/diesel 90%, wood chips 65%; 5% losses then 20% recovery. Dry matter is heated to 50 °C. Sunflower is heated to 45 °C. Operating time uses dried grain mass. Motor power already includes the 0.8 factor.'));
