...
export function calculateEngineering(input, data) {
  ...
  const optional = [input.fuelPrice, input.electricityPrice, input.elevatorTariff, input.dryerPrice, input.installation];
  if (!model || !crop || !fuel || !positive(input.volume) || input.volume > 1e9 ||
  ...
  const capacity = point?.capacity ?? (reference?.input === input.initialMoisture && reference?.output === input.finalMoisture ? reference.capacity : null);
  ...
  const thermal = model.thermalApproved && positive(data.waterHeat) && positive(crop.grainHeat) && positive(crop.latentHeat) &&
    nonnegative(crop.waterDelta) && nonnegative(crop.grainDelta) && positive(model.burnerEfficiency) && model.burnerEfficiency <= 1 &&
  ...
  if (![model.operatorPerHour,model.maintenancePerHour,model.seasonalFixed].every(nonnegative)) missing('OPERATING_RATES');
  ...
  const flow = (volume) => {
    ...
    // Aggregated model from section 57; approval is required to enable this branch.
    const usefulMJ = thermal ? waterKg * data.waterHeat * crop.waterDelta + waterKg * crop.latentHeat + dryMatterKg * crop.grainHeat * crop.grainDelta : null;
    const burnerMJ = thermal ? usefulMJ * (1 + model.lossFactor - model.recoveredFraction) / model.burnerEfficiency : null;
    ...
    const hours = r.capacity ? volume / r.capacity : null;
    ...
    const operatorCost = nonnegative(model.operatorPerHour) ? product(hours,model.operatorPerHour) : null;
    const maintenanceCost = nonnegative(model.maintenancePerHour) ? product(hours,model.maintenancePerHour) : null;
    // Single seasonal fixed expense, allocated by throughput between identical regimes.
    const fixedCost = nonnegative(model.seasonalFixed) ? model.seasonalFixed * volume / totalVolume : null;
